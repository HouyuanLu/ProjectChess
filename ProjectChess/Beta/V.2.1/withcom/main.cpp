...
//todo: ERROR: cnt counting problem(Not recognizing 4/3inarow): Attack or CheckAI or CheckVis
    ...
    if(tmpa == 1){
        while(true){
            ...
            if(tmpb == 1){
                ...
                cout << "Note: Enter the row you want to place the piece first, then enter the column." << endl;
                cout << "EXP: If you want to place on the second row in the third column, type this '2 3'." << endl;
                cout << endl;
            }
            if(tmpb == 2){
...
void defend(int x, int y, int MIN_LENGTH){
    ...
    for(int i = 0; i<8; i++){
        ...
        cout << "New defendtmp: " << defendtmp << endl;
        if(defendtmp >= MIN_LENGTH){
            cout << "defendtmp in MIN_LENGTH" << endl;
            if(defendtmp == 300){
                cout << "defendtmp continued" << endl;
                continue;
            }
            if(defendtmp > m){
                if(defendtmp >= MIN_LENGTH){
                    m = defendtmp;
                    dway = i;
                    cout << "m changed: " << m << endl;
                    cout << "dway changed: " << dway << endl;
                }
            }
        }
    }
        ...
        if(vis[x+dx[i]][y+dy[i]] == 2&&x+dx[i]>= 1 && x+dx[i] <= 20 && y+ dy[i] >= 1 && y+dy[i] <=20){
            ...
            for(int i1 = 0; i1 <= 6; i1++){
                ...
                if(xx >= 1 && yy >= 1 && xx <= 20 && yy <= 20){
                    ...
                    if(vis[xx][yy] != 2){
                        cout << "Found block at: " <<xx << " " << yy << endl;
                        int xxx = xx;
                        int yyy = yy;
...
int main() {
    ...
    while (steps <= 50000) {
        ...
        cin >> tmpxxx >> tmpyyy;
        if(stoi(tmpxxx) > 20 || stoi(tmpxxx) < 1 || stoi(tmpyyy) > 20 || stoi(tmpyyy) < 1){
